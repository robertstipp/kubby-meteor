...
const clusterMetricsController: clusterMetricsController = {
  getClusterInfo: async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      return next({
        log: 'Error occurred obtaining getting cluster info data',
        status: 400,
        message: { error: 'Error in clusterMetricsController' },
      });
    }
  },
  getClusterMetrics: async (
    ...
    try {
      ...
      return next({
        log: 'Error occurred obtaining getting cluster metrics (not flat) data',
        status: 400,
        message: { error: 'Error in clusterMetricsController' },
      });
    }
  ...
  getFlatClusterMetrics: async (
    ...
    try {
      ...
      response.items.forEach((pod) => {
        ...
        pod.containers.forEach((container) => {
          ...
          clusterUsage[containerKey] = {
            cpuUsage: convertKiBToMB(cpuNum),
            memUsage: convertKiBToMB(memoryNum),
            type: 'container',
          };
          podUsageObj.cpuUsage += cpuNum;
          podUsageObj.memUsage += memoryNum;
        ...
        pod.containers.forEach((container) => {
          ...
          clusterUsage[containerKey].cpuUsagePct =
            (clusterUsage[containerKey].cpuUsage / podUsageObj.cpuUsage) *
              100 || 0;
      ...
      for (const [key, value] of Object.entries(nodeMap)) {
        ...
        for (const pod of pods) {
          ...
          clusterUsage[pod.podName] = {
            cpuUsage: convertKiBToMB(pod.cpuUsage),
            cpuUsagePct: pod.cpuPct,
            memUsage: convertKiBToMB(pod.memUsage),
        ...
        const nodeObj = { name: key, ...nodeMap[key] };
        clusterUsage[key] = {
          cpuUsage: convertKiBToMB(nodeObj.cpuUsage),
          memUsage: convertKiBToMB(nodeObj.memUsage),
          type: 'node',
        };
      }
      ...
      return next({
        log: 'Error occurred obtaining getting flat cluster metrics data',
        status: 400,
        message: { error: 'Error in clusterMetricsController' },
      });
    }
